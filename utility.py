...
def save_image(reconstructed_image_, x,count=0):
    for t in range(T):
    ...
    for t_rec in range(T):
        img_rec = xrecons_grid(reconstructed_image_[t],B,A)
        plt.matshow(img_rec, cmap=plt.cm.gray)
        imgname_recon = 'image/reconstruction_%d_%s_%d.png' % (count,'test', t)  # you can merge using imagemagick, i.e. convert -delay 10 -loop 0 *.png mnist.gif
        plt.savefig(imgname_recon)
        print(imgname_recon)
